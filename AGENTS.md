...
**Update notice:** At session start, when `.forge/releases.py` exists, run `python3 .forge/releases.py check --quiet` (`py` on Windows). Relay a returned update notice once, then continue the user's task. This is a cached, best-effort check, not permission to upgrade. Skip it if Python or network access is unavailable; never request permission escalation or retry for an automatic check. `RESONANCE_NO_UPDATE_CHECK=1` disables discovery. `/update-resonance` previews the latest stable release and requires approval before applying it.
