...
### Added
- **The full library scored (`docs/EVAL_SCORECARD.md`).** All 57 skills run cold versus skill-applied and graded per rubric: mean lift **+0.68**, every skill showing measured lift over the base model. The doc states the honest caveats: one case per skill this run, and the `with` column is near-ceiling because each rubric is authored alongside its skill, so `without` and the lift are the real signal. The `/improve` dogfood shows on the board: `ops/ship` was flat before its rubric was sharpened and measures +0.80 here.
